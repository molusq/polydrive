 ...
 * Last file update: 22-Dec-2017 14:28 (eg)
 */
...
%token KWHILE KFOR KIF KPRINT KRETURN
%token TINT TBOOL TSTRING TVOID
%token MM PP KBREAK KPOW
%token KTRY KCATCH KFIN


%type   <ast>   declarations declaration
%type   <ast>   stmt expr var_decl type var call
%type   <ast>   func_decl prototype expr_opt init_var for_init for_expr
...
stmt:           ';'                     { $$ = make_expr_statement(NULL); }
        ...
        |       KFOR '(' for_init ';' for_expr ';' for_expr ')' stmt
                                        { $$ = make_for_statement($3, $5, $7, $9);}
        |       KTRY '{' stmt '}' KCATCH '{' stmt '}' %prec KFINX
                                        { $$ = make_try_statement($3, $7, NULL); }
        |       KTRY '{' stmt '}' KCATCH '{' stmt '}' KFIN '{' stmt '}'
                                        { $$ = make_try_statement($3, $7, $11); }
...
expr:
        ...
        |       PP var                { $$ = make_expression("++", prefix, 1, $2);}
        |       var PP                { $$ = make_expression("++", suffix, 1, $1);}
        |       MM var                { $$ = make_expression("--", prefix, 1, $2);}
        |       var MM                { $$ = make_expression("--", suffix, 1, $1);}
        |       expr KPOW expr        { $$ = make_expression("**", barith, 2, $1, $3);}
        ;
