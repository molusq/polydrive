...
ast_node *int_type, *bool_type, *string_type, *void_type;
...
static void free_if_statement(ast_node *node) {
    ...
    free_node(n->cond);
    free_node(n->then_stat);
    free_node(n->else_stat);
    free(node);
}
...
static void free_expr_statement(ast_node *node) {
    free_node(((struct s_expr_statement *) node)->stat);
    free(node);
}
...
static void free_return_statement(ast_node *node) {
    free_node(((struct s_return_statement *) node)->expr);
    free(node);
}
...
ast_node *make_for_statement(ast_node *init, ast_node *cond, ast_node *action, ast_node *body) {
...
static void free_try_statement(ast_node *node) {
    ...
    free_node(n->_try);
    free_node(n->_catch);
    free_node(n->_finally);
    free(node);
}

ast_node *make_try_statement(ast_node *_try, ast_node *_catch, ast_node *_finally) {
