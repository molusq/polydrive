        ...
        case 1:
            ...
            switch (EXPRESSION_KIND(node)) {
                ...
                case prefix:
                    ...
                    error_msg(node, "operand of unary %s must be int or string", name);
                    break;
                default:
...
void analysis_if_statement(ast_node *node) {
    ...
    if (AST_TYPE(n->cond) != bool_type) {
        error_msg(node, "Error in if condition");
    }
...
void analysis_while_statement(ast_node *node) {
    ...
    analysis(n->cond);
    if (AST_TYPE(n->cond) != bool_type) {
        error_msg(node, "Error in while condition");
    }
    // Analyze loop body
    analysis(n->body_stat);
    if (AST_KIND(n->body_stat) == k_break_statement) {
        leave_scope();
    }
    k_break--;
}
...
void analysis_break_statement(ast_node *node) {
    if (k_break == 0) {
        error_msg(node, "break forbidden here");
    }
}
...
void analysis_for_statement(ast_node *node) {
    ...
    // Analyze condition
    analysis(n->cond);
    if (n->cond && AST_TYPE(n->cond) != bool_type) {
        error_msg(node, "Error in for condition");
    }
    ...
    // Analyze loop body
    analysis(n->body_stat);
    if (n->body_stat && AST_KIND(n->body_stat) == k_break_statement) {
        leave_scope();
    }
    k_break--;
    leave_scope();
