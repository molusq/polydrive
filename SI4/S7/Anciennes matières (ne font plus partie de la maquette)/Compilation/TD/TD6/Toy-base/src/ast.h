...
enum node_kind {
    k_constant, k_identifier, k_type, k_var_decl, k_expression,
    k_call, k_function, k_parameter_list,
    k_if_statement, k_while_statement,
    k_block_statement, k_expr_statement, k_print_statement,
    k_return_statement, k_break_statement, k_for_statement, k_try_statement
};
...
struct ast_node {
    ...
    void (*produce_code)(ast_node *

    this); ///< method for producing code
    void (*analysis)(ast_node *

this);     ///< method for node analysis
    void (*free)(ast_node *

this);         ///< method for freeing a node
#ifdef DEBUG
...
// ======================================================================
enum expr_kind {
    assign, uarith, barith, ulogic, blogic, comp, ternary, parenthesis, prefix, suffix
};
...
ast_node *make_break_statement();

/* ---- FOR -------------------------------------------------------------- */
struct s_for_statement {
    ast_node header;              ///< AST header
    ast_node *init;               ///< for init AST
    ast_node *cond;               ///< for cond AST
    ast_node *action;             ///< for action AST
    ast_node *body_stat;          ///< while body AST
};

ast_node *make_for_statement(ast_node *, ast_node *, ast_node *, ast_node *);

/* ---- TRY -------------------------------------------------------------- */
struct s_try_statement {
    ast_node header;              ///< AST header
    ast_node *_try;               ///< for init AST
    ast_node *_catch;               ///< for cond AST
    ast_node *_finally;             ///< for action AST
};

ast_node *make_try_statement(ast_node *, ast_node *, ast_node *);
