    ...
    switch (EXPRESSION_ARITY(node)) {
        ...
        case 2:
            if (EXPRESSION_KIND(node) == comp &&
                ...
                code(EXPRESSION_OP2(node));
                emit(")");
            } else if (AST_TYPE(node) == string_type && strcmp(name, "+") == 0) {
                emit("_toy_concat(");
                code(EXPRESSION_OP1(node));
                emit(", ");
                code(EXPRESSION_OP2(node));
...
void produce_code_for_statement(ast_node *node) {
    ...
    indent(0);
    emit("while (");
    if (n->for2) code(n->for2); else emit("1");
    emit("){\n");
...
void produce_code_switch_statement(ast_node *node) {
    ...
    {
        emit("if(");
        produce_code(list_item_data(p));
        emit(")\n");
        indent(+1);
        p = list_item_next(p); // Permet de passer directement au contenu du case
        // Fonctionne car on a forcément un contenu dans le case
        // Autrement ne traiter qu'un par un les éléments de la liste
        produce_code(list_item_data(p));
        indent(-1);
    }
    if (n->def_cond) {
        if (list_size(n->parameters) != 0) {
            emit("else {\n");
            indent(+1);
            produce_code(n->def_cond);
            indent(-1);
            emit("}\n");
        } else produce_code(n->def_cond);
    }
    indent(-1);
}
