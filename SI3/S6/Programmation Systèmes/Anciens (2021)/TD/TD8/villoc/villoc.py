...
class Block(Printable):
	...
	def more_html(self):
		return "+ %#x (%d)" % (self.end() - self.start(), self.usize)
...
class SuperBlock(Block):
	...
	def __init__(self, addr, size, error=False, tmp=False, **kwargs):
		super().__init__(addr, size, tmp=True, error=error, *kwargs)
		...
		try:
			...
			try:
				...
				try:
					...
					if (func_stack != func):
						raise RuntimeError('Function on stack does not match found one... Parsing problem!')
				except Exception:
					try:
...
if __name__ == '__main__':
	...
	if args.raw:
		...
		Block.header, Block.footer, Block.round, Block.minsz = (args.header, args.footer, args.round, args.minsz)
