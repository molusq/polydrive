...
public class McDoReaderToFill {
	...
	long nbCitiesHaveMcDo() {
		return this.restaurants.stream().map(Restaurant::getCity).distinct().count();
	}
	...
	List<Restaurant> restaurantsInACity(String city) {
		return this.restaurants.stream().filter(r -> r.getCity().equals(city)).toList();
	}
	...
	int nbOfMcDoInACity(String city) {
		return this.restaurants.stream().filter(r -> r.getCity().equals(city)).toList().size();
	}
	...
	int nbOfMcDoInState(String state) {
		return this.restaurants.stream().filter(r -> r.getState().equals(state)).toList().size();
	}
	...
	String cityHasMostMcDo() {
		return this.restaurants.stream().collect(Collectors.groupingBy(Restaurant::getCity, Collectors.counting()))
				       .entrySet().stream().max(Map.Entry.comparingByValue()).orElseGet(() -> new AbstractMap.SimpleEntry<>("no result", 0L)).getKey();
	...
	String cityWithFewestMcDo() {
		return this.restaurants.stream().collect(Collectors.groupingBy(Restaurant::getCity, Collectors.counting()))
				       .entrySet().stream().min(Map.Entry.comparingByValue()).orElseGet(() -> new AbstractMap.SimpleEntry<>("no result", 0L)).getKey();
	}
	...
	List<String> cityListWithOneMcDo() {
		return this.restaurants.stream().collect(Collectors.groupingBy(Restaurant::getCity, Collectors.counting()))
				       .entrySet().stream().filter(e -> e.getValue() == 1).map(Map.Entry::getKey).sorted(Comparator.reverseOrder()).limit(15).toList();
	}
	...
	List<Map.Entry<String, Long>> fourCitiesWithMostMcDo() {
		return this.restaurants.stream().collect(Collectors.groupingBy(Restaurant::getCity, Collectors.counting()))
				       .entrySet().stream().sorted(Map.Entry.comparingByValue(Comparator.reverseOrder())).limit(4).toList();
	}
	...
	private List<String> cityListInState(String state) {
		return this.restaurants.stream().filter(r -> r.getState().equals(state)).map(Restaurant::getCity).distinct().sorted().toList();
	}
