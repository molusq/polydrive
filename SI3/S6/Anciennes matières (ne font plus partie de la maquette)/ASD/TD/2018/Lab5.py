...
class BinaryHeap:
	...
	def smaller(self, i, j):
		return i < len(self._array) and j < len(self._array) and self.key(
			self._array[i]) < self.key(self._array[j])
	...
	def buildHeap(self):
		...
		for n in range(len(self) // 2 - 1, -1, -1):
			# //2 car on ne prend pas les feuilles
...
class DHeap(BinaryHeap):
	...
	def __init__(self, D, init_array=None, key=lambda x: x):
		if init_array is None:
			init_array = []
		self.D = D
		self._array = list(init_array)
...
class TestLab5(unittest.TestCase):
	...
	@unittest.skipIf(isnt_implemented(DynamicMedian.pop_median), "BinaryHeap.add not implemented")
	def test7_add(self):
		for i in range(1, 20):
			dm = DynamicMedian()
			witness = []
