...
class BST:
    ...
    def find_extremum(self, maxi=False):
        ...
        '''
        if self.root is None: raise ValueError("find_extremum : empty BST")
        node = self.root
        while node[maxi] is not None: node = node[maxi]
        return node.data
    ...
    def remove_extremum(self, maxi=False, node=None):
        ...
        if node is None: raise ValueError("remove_extremum : empty BST")
        while node[maxi] is not None: node = node[maxi]
        v = node.data
    ...
    def remove(self, value):
        ...
        '''
        node = self.root
        while node is not None and node.data != value: node = node[value > node.data]
        if node is None: return False
        for s in False, True:
            if node[s] is None:
                node.replace(node[not s])
                return True
        v = self.remove_extremum(node=node[True])
        node.data = v
        return True
    ...
    def remove_compare(self, value, greater=False):
        ...
        node = self.root
        while True:
            while node is not None and not compare(node.data, value): node = node[greater]
            if node is None: return
            pnode = node
            while node is not None and compare(node.data, value): node = node[not greater]
            pnode.replace(node)
...
class RankedBST(BST):
    ...
    def insert(self, value):
        ...
        while True:
            if value == node.data: return False
            cond = value > node.data
            if not cond: incr.append(node)
            if node[cond] is None:
                node[cond] = self.__class__.Node(value)
                for n in incr: n.sizeOfLeft += 1
                return True
            node = node[cond]
    ...
    def remove(self, value):
        ...
        while node is not None and node.data != value:
            if value < node.data: decr.append(node)
            node = node[value > node.data]
        if node is None: return False
        for n in decr: n.sizeOfLeft -= 1
        for s in False, True:
            if node[s] is None:
                node.replace(node[not s])
                return True
    ...
    def __getitem__(self, rank):
        ...
        def aux(node, rank):
            if node is None: raise IndexError('RankedBST index out of bound')
            if rank < node.sizeOfLeft: return aux(node[False], rank)
            if rank == node.sizeOfLeft: return node.data
            return aux(node[True], rank - node.sizeOfLeft - 1)

        return aux(self.root, rank)
    ...
    def index(self, value):
        ...
        def aux(node, accu):
            if node is None: raise ValueError('RankedBST value not found')
            if value == node.data: return accu + node.sizeOfLeft
            if value < node.data: return aux(node[False], accu)
            return aux(node[True], accu + node.sizeOfLeft + 1)
