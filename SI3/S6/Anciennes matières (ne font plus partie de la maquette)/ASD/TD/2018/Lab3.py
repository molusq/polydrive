...
def balanced1(tree):
	...
	return tree is None or (
			abs(height(tree.left) - height(tree.right)) < 2 and balanced1(tree.left) and balanced1(tree.right))
...
def balanced2(tree):
	...
	def aux(t):  # renvoie la hauteur si équilibré
		if t is None:
			return -1
		hl, hr = aux(t.left), aux(t.right)
		return None if hl is None or hr is None or abs(hl - hr) > 1 else max(hl, hr) + 1
...
class TestLab2(unittest.TestCase):
	...
	def testD_isomorphic(self):
		...
		self.assertTrue(isomorphic(build_tree("A ! B"), build_tree("B ! A")))
		self.assertFalse(isomorphic(build_tree("A ! B"), build_tree("B A")))
		self.assertFalse(isomorphic(build_tree("A ! B ! A"), build_tree("B ! A B")))
		self.assertFalse(isomorphic(build_tree("A C ! ! B ! A"), build_tree("B C ! ! A B")))
