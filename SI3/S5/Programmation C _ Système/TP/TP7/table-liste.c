...
// Cette implémentation utilise une liste triée simplement chaînée. Pour simplifier, 
// nous allons calquer cette première implémentation sur celle des liste de la feuille 
// précédente. Nous allons donc définir la structure t_table de la façon suivante.
...
// Du coup, la création de la table se résume à:
...
// La seule fonction vraiment «compliquée» est la fonction d’ajout dans la table. Une 
// implémentation possible pourrait être:
// Notes:
//   -  La fonction de recherche tire partie du fait que la liste est triée (on ne 
//      parcourt pas les éléments supérieurs au mot recherché);
//   - La fonction d’impression utilise la fonction appliquer_table.
